...
{
  public:
    RISCVDAGToPatterns(RISCVTargetMachine& TM, CodeGenOptLevel OptLevel) : RISCVDAGToDAGISel(TM, OptLevel)
    {
    }
...
FunctionPass* createRISCVPatternsISelDag(RISCVTargetMachine& TM, CodeGenOptLevel OptLevel)
{
...
void optimizeModule (llvm::TargetMachine* machine, llvm::Module* module, size_t opt_level)
{
    ...
    ModulePassManager MPM = PB.buildPerModuleDefaultPipeline(llvm::OptimizationLevel::O3);
...
int RunPatternGenPipeline(llvm::Module* M, std::string mattr, size_t opt_level)
{
    ...
    std::string error;
    const class Target* TheTarget = llvm::TargetRegistry::lookupTarget(codegen::getMArch(), TheTriple, error);
